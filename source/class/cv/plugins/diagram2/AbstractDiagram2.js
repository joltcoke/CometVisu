  ...
  statics: {
    ...
    getDiagramElements(xmlElement) {
      ...
      xmlElement.querySelectorAll('influx,rrd,iobroker').forEach(function (elem) {
        const src = ['rrd', 'iobroker'].includes(elem.tagName) ? elem.textContent : elem.getAttribute('measurement');
        ...
        retVal.ts[retVal.tsnum] = {
          ...
          cFunc: elem.getAttribute('consolidationFunction') || ['rrd', 'iobroker'].includes(elem.tagName) ? 'AVERAGE' : 'MEAN',
    ...
    lookupTsCache(ts, start, end, series, period, res, forceNowDatapoint, refresh, force, callback, callbackParameter) {
      switch (ts.tsType) {
        ...
        case 'iobroker':
          ts.source = new cv.io.timeseries.IoBrokerSource('iobroker://' + ts.src);
          break;
    ...
    _onStatusError(ts, text, callback, callbackParameter) {
      cv.core.notifications.Router.dispatchMessage('cv.diagram2.error', {
  ...
  members: {
    ...
    loadDiagramData(plot, isInteractive, forceReload) {
      ...
      this.getContent().ts.forEach(function (ts, index) {
        ...
        cv.plugins.diagram2.AbstractDiagram2.lookupTsCache(
          ts,
          series.start,
          series.end,
          this.getSeries(),
